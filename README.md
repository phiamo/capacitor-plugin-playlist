...
| **`stallTimeoutMs`**     | <code>number</code>                                                 | How long, in ms, playback position may fail to advance while the player reports it is playing before the plugin declares the track stalled and emits `RMXSTATUS_STALLED` (issue #143). All three platforms use this as the threshold for their position-freeze backstop, in case the platform's own native "playing" signal fails to fire. Default is 10000.                                                             |
