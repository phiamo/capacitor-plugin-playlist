...
| **`stallTimeoutMs`**     | <code>number</code>                                                 | (Android only) How long, in ms, `currentPosition` may fail to advance while the player reports it is playing before the plugin declares the track stalled and emits `RMXSTATUS_STALLED` (issue #143). iOS and web rely on native stall notifications instead and ignore this option. Default is 10000.                                                                                                                    |
