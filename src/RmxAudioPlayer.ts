...
export class RmxAudioPlayer {
  ...
  addItem = (trackItem: AudioTrack) => {
    ...
    if (!validTrackItem) { throw new Error('Provided track is null or not an audio track'); }
  ...
  removeItem = (removeItem: AudioTrackRemoval) => {
    if (!removeItem) { throw new Error('Track removal spec is empty'); }
    if (!removeItem.trackId && !removeItem.trackIndex) { new Error('Track removal spec is invalid'); }
